module.exports = (function() {
	...
	function getTag(name, attr, parent, tType, tagTypes) {
		var tagName = "";
		// Work on square root, subscript and root.
		...
		switch(name) {
				...
				case tags.root: tagName = "Work on it!" + ","; break;
				case tags.row: tagName = signDisplay(tType, "(", ")", tagTypes); break;
				case tags.squareRoot: tagName = "Work on it!" + ","; break;
	...
	function placeSubscript(subCount) {
		if (subCount.placed) {
			return "";
		}
		
		if (subCount.NODE_COUNT === 0) {
			subCount.NODE_COUNT++;
		} else {
			subCount.placed = !subCount.placed;
			return "_{";
		}
	...
	function placeOperator(parent) {
		...
		switch(parent.name) {
			...
			case tags.subscript: symbols = placeSubscript(parent.otherAttr); break;
		}
