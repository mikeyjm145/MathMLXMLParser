...
var mathMLToRegMathParser = (function (script) {
	...
	function getOtherAttributes(name) {
		if (name === tags.frac) {
			...
			return { NODE_COUNT: 0, placed: false, baseLevel: 0, currLevel: 0 };
		}
	...
	function addNode(name, attr, parent, currLevel, otherAttr) {
        ...
        if (name === tags.superscript || name === tags.subscript) {
			otherAttr.baseLevel = heirarchy.level;
		}
	}
	...
	function parseOpenTag(node) {
		if (heirarchy.openTags.length === 0) {
            ...
            if (parent.name === tags.superscript || parent.name === tags.subscript) {
                parent.otherAttr.currLevel = heirarchy.level;
            }
	...
	var parserForMMLToRM = function (saxParser, script, strict) {
